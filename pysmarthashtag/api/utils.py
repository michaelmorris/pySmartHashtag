...
def _create_sign(authentication: AbstractAuthentication, nonce: str, params: Dict, timestamp: str, method: str, url: str, body=None) -> str:
...
{url}"""
    ...
    if (isinstance(authentication, VolvoAuthentication)):
        secret = base64.b64decode("MGU0MzFhZGY0YmY5NGE2YWI3YmUyYzY4NjhkNGMwNjQ=")
    if (isinstance(authentication, SmartAuthentication)):
...
def generate_default_header(
    authentication: AbstractAuthentication, params: Dict, method: str, url: str, body=None
) -> Dict[str, str]:
    """Generate a header for HTTP requests to the server."""
    timestamp = create_correct_timestamp()
    nonce = secrets.token_hex(8)
    device_id = authentication.device_id
    access_token = authentication.api_access_token
    sign = _create_sign(authentication, nonce, params, timestamp, method, url, body)
    if (isinstance(authentication, VolvoAuthentication)):
        header = {
